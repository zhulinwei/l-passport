...
class Passport {
  ...
  use(name, strategy) {
    if (!name || typeof name !== 'string') 
      throw new PassportError('Authentication strategies must have a name');
    if (!strategy || typeof strategy !== 'function') 
      throw new PassportError('Authentication strategies must be a function');
  ...
  initialize(configs) {
    ...
    for (let config of configs) {
      ...
      if (!provider) 
        throw new PassportError('provider missing');
      if (this._configs[provider]) 
        throw new PassportError(`provider ${provider} already exist`);
      if (!this._strategies[provider]) 
        throw new PassportError(`the strategy of ${provider} is not existed`);
      if (!config.appId && !config.appSecret && !config.clients) 
        throw new PassportError('appId or appSecret missing');
  ...
  authorization(provider, options) {
    ...
    if (!provider || !util.isString(provider)) 
      throw new PassportError('provider name must be a string');
    
    if (options && !util.isObject(options)) 
      throw new PassportError('option must be object');
    ...
    return async function authorization(ctx, next) {
      ...
      const config = getProviderConfig(passport._configs[provider]) || {};
      ...
      const strategy = new passport._strategies[provider](config);
