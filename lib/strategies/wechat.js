...
class WechatStrategy extends Base {
  ...
  async getTokenByCode(code, appId, secret) {
    ...
    if (!result || !result.access_result && !result.openid) {
  ...
  async authorize(code) {
    ...
    const { openId, accessToken, refreshToken } = await this.getTokenByCode(code, appId, secret);
